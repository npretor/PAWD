...
void loop() {
    ...
    if (newData == true) {
        ...
        if (prevTrigger == triggerValue){
          ...
          // If the incoming value changes, and it's 1, start the timer 
          if (triggerValue == 1){
            // Fire();
...
// Start firing timer
void Fire(){
  unsigned fire_milliseconds = 2000; 
  stop_time = millis() + fire_milliseconds;
}

void updateFiringStatus(){
  if ((millis() > stop_time) && (firingActive == 0)){
    // Do nothing 
  }
  else if ((millis() > stop_time) && (firingActive == 1)){
    firingActive = 0;
    digitalWrite(TRIGGER_PIN, LOW);
  } 
  // Assume we want to fire 
  else {
    firingActive = 1;
    digitalWrite(TRIGGER_PIN, HIGH);
  }
